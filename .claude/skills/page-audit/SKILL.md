...
Both measurements run against the files exactly as the host serves them. The site has no
build step, so serving the repository root is the production build.

## 1. Serve the site

Start a static server at the repository root (`python3 -m http.server 8000`) and note its
URL. If the port is occupied, kill the holder (`lsof -ti:<port> | xargs kill`) rather than
...
4xx/5xx. Do not measure a page you have not seen render.

## 3. Pick the targets

With `$path`, audit that page alone. Otherwise audit every `.html` file the repository
serves, which today is `index.html` alone.
...
In priority order: render-blocking resources (inline or preload what the first paint
needs), LCP (preload the element, cut server response time), CLS (explicit dimensions,
nothing inserted above the fold). Reload the page before re-measuring, and add the
after-fix numbers to the same report.
