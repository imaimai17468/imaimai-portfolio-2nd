...
- **Two colors, text only.** The page uses the wairo 暗黒色 (`--ankoku`, `#16160e`) and 白磁 (`--hakuji`, `#f8fbf8`): 暗黒色 text on 白磁 in light mode, swapped under `prefers-color-scheme: dark`. The avatar is its one image. Keep colors as custom properties on `:root` in `style.css`.
