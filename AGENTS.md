...
This repository is a static site: `index.html`, `style.css`, `icon.png` (the favicon and the header avatar), and `og.png` (the 1200×630 share image for OG and Twitter Card) at the repository root, and nothing else the browser loads apart from the Roboto web font. There is no package manager, build step, framework, JavaScript, test suite, linter, or CI. Vercel serves the repository root with the "Other" framework preset, and a merge to `main` deploys it.
...
- **Two colors, text only.** The page uses the wairo 暗黒色 (`--ankoku`, `#16160e`) and 白磁 (`--hakuji`, `#f8fbf8`): 暗黒色 text on 白磁 in light mode, swapped under `prefers-color-scheme: dark`. The avatar is the one image on the page; `og.png` appears only in link previews. Keep colors as custom properties on `:root` in `style.css`.
